...
float roundNum(float f)
{
	...
	fCarry = 0.5;
	i = (int)f;
	if (f - i >= fCarry) // Nếu phần thập phân của f (lúc này) lớn hơn 0.5 thì làm tròn
		i ++;
	f = (float)i/100000; // Trả lại kết quả tương đương f ban đầu (sau khi làm tròn)

	return f;
}

// Hàm xuất đường đi cần tìm và độ dài đường đi ngắn nhất (hoặc thời gian ngắn nhất)
