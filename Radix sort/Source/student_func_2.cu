...
__global__ void histogram(unsigned int* inData, unsigned int* hist, int n)
{
  ...
  if (i < n)
  {
    atomicAdd(&hist[inData[i]*gridDim.x+blockIdx.x], 1);
  } 
}
...
// Each threads in block computes rank of an element.
//=====================================================================
__global__ void computeRank_1(unsigned int* inData, unsigned int* outData, int n)
{
  ...
  if (idx < n)
  {
    ...
    for (int i = 0; i < threadIdx.x; i++)
      rank += (s_inData[i] == s_inData[threadIdx.x]);
...
// Compute rank of each nBits-digit
// Each blocks (just contain 1 thread) computes rank of a number of elements
//=====================================================================
__global__ void computeRank_2(unsigned int* inData, unsigned int* outData,
			    int n, int nSize, int nBins)
{
  ...
  // Computes rank of 'nSize' elements
  for (int i = 0; i < nSize; i++)
  {
    int idx = blockIdx.x * nSize + i;
    if (idx < n)
    {
      unsigned int bin = inData[idx];
      outData[idx] = s_bin[bin];
      s_bin[bin]++;
    }
  }
}
...
{ 
  ...
  for (unsigned int i = 0; i < 8*sizeof(unsigned int); i += nBits)
  {
    ...
    histogram<<<gridSize, blockSize>>>(digit, hist, numElems);
    cudaDeviceSynchronize(); checkCudaErrors(cudaGetLastError());
    ...
    computeRank_1<<<gridSize, blockSize, blockSize.x*sizeof(unsigned int)>>>(digit, rank, numElems);
    
    //computeRank_2<<<gridSize, 1, nBins*sizeof(unsigned int)>>>
    //							(digit, rank, numElems, blockSize.x, nBins);
    // In the best case of each kernel, the latency time of computeRank_2 kernel
    // still lower than the latency time of computeRank_1.
    // So we choose the computeRank_1 kernel for rank problem
